...
function epm_mailchimp_submit_to_list() {
	...
	if($epm_enable_validation == 'enabled') {
		...
		if(empty($epm_name) && epm_get_option('display_name_fields')) {
			echo '<div class="epm-message epm-error message error"><p>'.__('Please fill in name field.','easy-peasy-mailchimp').'</p></div>';
		}
		// email field is empty and is not an email
		if(empty($epm_email) && !is_email( $epm_email )) {
			echo '<div class="epm-message epm-error message error"><p>'.__('Please add a correct email address.','easy-peasy-mailchimp').'</p></div>';
		}
	...
	jQuery('.epm-submit-chimp').click(function() {
		...
		jQuery.ajax({
			...
			success: function(data, textStatus, XMLHttpRequest){
				...
				if ( jQuery( data ).hasClass( "success" ) ) {
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <?php $popup_title = get_option( 'popup_title', '' ); ?>
        <h4 class="modal-title"><strong><?php echo  $popup_title; ?></strong></h4>
      </div>
      <div class="modal-body">
      	<?php $popup_text = get_option( 'popup_text', '' ); ?>
